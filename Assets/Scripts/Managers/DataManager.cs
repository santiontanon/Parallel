...
public class DataManager : MonoBehaviour {
	...
	public void InitializeLoadLevel(string inputLevelName, bool resources = false)
	{
		levelname = inputLevelName;
        Debug.Log("Loading level " + inputLevelName);
		TextAsset bindata = new TextAsset();

        //this is a bad way to test if it's receiving a level name or some path to a level
		if(inputLevelName[0] == 'l') 
		{
			bindata = Resources.Load(foldername+levelname) as TextAsset;
