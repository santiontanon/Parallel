...
public class GrammarStatsCurrentLevelSolutions {
    ...
    public double[] processFile2(File file) throws Exception {
        GameState gs;
        System.out.println("Processing: " + file.getAbsolutePath());
        gs = GameStateParser.parseFile(file.getAbsolutePath(), false);
        return GrammarStats.getVisualPropertiesVector(gs);
    }
    ...
    public double[] processFiles2(int seed, int size) throws Exception {
        ...
        return processFile2(out_file_ns);
    }

    
    public void run2() throws Exception {

        File out_file = new File("/Users/josepvalls/temp/pppp_samples/", WRITE_ARFF ? "dataset.arff" : "dataset3.txt");
        PrintWriter writer = new PrintWriter(out_file);
        double[] row;
        double[] stats_maximums = new double[15];
        double[] stats_averages = new double[15];
        int stats_count = 0;
        ...
        //for (int level = 5; level < 6; level++) {
            System.out.println("LEVEL" + level);
            File file = new File(path, "level0" + level + ".txt");
            row = processFile2(file);
            stats_count++;
            for (int j = 0; j < row.length; j++) {
                writer.print(row[j]);
                if(row[j]>stats_maximums[j]) stats_maximums[j]=row[j];
                stats_averages[j] += row[j];
                if(WRITE_ARFF){
                    writer.print(',');
                } else {
                    writer.print('\t');
        ...
        for (int j = 0; j < stats_averages.length; j++) {
            stats_averages[j]=(stats_averages[j]/stats_count)/stats_maximums[j];
        }

        
        System.out.println("Maximums");
        for (int j = 0; j < stats_averages.length; j++) {
            System.out.print(stats_maximums[j]);
            System.out.print(", ");
        }
        System.out.println("");
        System.out.println("Normalized Averages");
        for (int j = 0; j < stats_averages.length; j++) {
            System.out.print(stats_averages[j]);
            System.out.print(", ");
        }
        System.out.println("");
